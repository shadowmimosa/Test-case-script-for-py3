...
def listContain(list1,list2):
	...
	if error:
		...
		for i in xrange(len(error)):
			logging.info(("非常遗憾！！在【目标list1字段集】中未发现list2中的【%s】字段！！！"%(error[i])).decode('utf-8'))
			pass
	return  ispas
...
def analysis_subItem(item,test):
	...
	if isinstance(item,str):
		...
		if int(item)<0:
		   ...
		   subkey=item.keys()
		   times=0
		   for i in range(0,value_dict_len):
			key=subkey[i]
			test.append(key)
			subvalue=item[subkey[i]]		 
			analysis_subItem(subvalue,test)
	return   test
